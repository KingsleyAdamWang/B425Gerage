...
public class InstitutionBL {
	...
	public String addIns(InstitutionVO insVO, InventoryVO invVO)
		...
		for (InstitutionPO temp : insList) {
			...
			if (temp.getInstitutionID().equals(vo.getInstitutionID())) {
				...
				for (UserPO tempUser : users) {
					if (tempUser.getInstitutionID().equals(
							vo.getInstitutionID())) {
						// 机构内删除人员
				...
				if (temp.getType() == InsType.intermediate) {
					...
					InventoryDataService invDS = RMIHelper
							.getInventoryDataService();
